...
function pars(L) {
  ...
  const min = 2n//exponents.reduce((a, b) => a < b ? a : b);
...
class JX1DX1Num {
  ...
  toNumber() {
    if (!this.arr[1][1]) return this.arr[0][1]*this.sign
    else return 10**this.arr[0][1]*this.sign
  }
  toString() {
    let res=""
    if (this.arr[1][1]) res += String(Math.floor(this.arr[0][1]))
